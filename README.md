...
4. Color: whether or not 40 percent or more of the pixels in an image are primarily of one color (red, orange, yellow, green, blue, purple, black, white, or gray). If not, then we categorize the image as â€œcolorfulâ€. Otherwise, the variable takes the name of the primary color.
...
1. Center: As shown in the diagram below, we evenly split each (resized ) image into 64 boxes and take the absolute difference between the average hue of the 16 boxes in the center (colored in orange) and that of the 48 boxes surrounding them. This feature would help us identify if thereâ€™s something in the center of the image that might be the point of focus.
![Center](Images/Center.png)

2. Edges: Measures how a pictureâ€™s edge varies in hue via standard deviation. The edge is defined as the outer 28 boxes . Some pictures include a frame, in which case there would be very small variability in the hue of the edge.
3. Value Mean: Value is a measure of lightness/darkness. The average of an imageâ€™s value across all of its pixels can give us information on the overall brightness of an image relative to other images.
4. Value Variance: Measures how much value varies among an imageâ€™s pixels. If there is low variability, then the pixels all tend to be either light or dark.
5. Saturation Mean: Saturation measures the intensity of a color. The mean of an imageâ€™s saturation across all of its pixels can give us information on the overall color intensity of an image relative to other images.
6. Saturation Variance: Measures how much saturation varies among an imageâ€™s pixels. If there is low variability, then the pixels all tend to have intense colors or not.
7. Dominance: Using the k-means clustering algorithm (described in the methods section), we formed 8 clusters for the pixels in an image based on their values of hue, saturation, value. We recognized that having only eight clusters to represent color might not be able to capture all the unique colors that can compose an image, but we decided that eight was a good number to represent the fundamental colors, since the most common retail packages for crayons are in multiples of eight. Dominance measures the proportion of pixels that are in the largest cluster. If an image has â€œdominanceâ€ of .8, that means 80 percent of the pixels of the image are similar in hue, saturation, and value.
...
We ran a k-means clustering algorithm for each of the three feature sets, creating 5, 6, and 2 clusters for the first, second, and third feature sets, respectively. We also created 9 clusters using the â€œcolorfulâ€ feature: red, orange, yellow, green, blue, purple, black, white, and colorful (which means the color scheme of an image isnâ€™t primarily of one color.)
...
"Complexity" measures the variability of color and brightness of an image. As we can see from the composite images (shown below), there is a clear difference between the most complex groups (whose composites are on the right end) and the least complex groups. Note that the â€œscaleâ€ we present here is not objective - one can easily rearrange the five composites based on how visually complex one perceives each one to be. We use letters to represent each composite, and the number in the parenthesis represents the size of the cluster. For example, A (114) means that composite A is composed of 114 images.
![Complexity](Images/Complexity.png)

### Personality 

As we can see from the composite images (shown below), each one has a different â€œpersonalityâ€ to it. For instance, composite E appears to be more gloomy than composite D.
![Personality](Images/Personality.png)

### Centrality 

Ideally, one of these two composites should not have any discernible edge. However, as shown below, both composites images have a clear separation between the central subject and the edge, with the first oneâ€™s central subject spreading out slightly more to its edges.
